...
export const useSessionStore = create((set, get) => ({
  ...
  startStream: (sessionId) =>
    set({
      ...
      streamingStatusLog: ['Understanding your question'],
  ...
  setStreamingStatus: (status) =>
    set((state) => {
      const log = state.streamingStatusLog;
      const last = log[log.length - 1];
      // Avoid pushing consecutive duplicate steps
      if (last === status) {
        return { streamingStatus: status };
      }
      return {
        streamingStatus: status,
        streamingStatusLog: [...log, status],
      };
    }),
  ...
  finalizeAiMessage: (sessionId, rawText, latencyMs) => {
    const { pendingCitationChunks, streamingStatusLog, streamStartTs } = get();
    const thinkingSeconds = streamStartTs
      ? Math.max(1, Math.round((Date.now() - streamStartTs) / 1000))
      : null;
    set((state) => ({
      chatSessionsMemory: {
        ...state.chatSessionsMemory,
        [sessionId]: [
          ...(state.chatSessionsMemory[sessionId] || []),
          {
            ...
            citationChunks: pendingCitationChunks,
            thoughtLog: streamingStatusLog,
            thinkingSeconds,
          }
        ],
      },
