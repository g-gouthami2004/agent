...
export function PdfViewerPanel() {
  ...
  const quotes = usePdfStore((s) => s.quotes) || [];
  const answerSentence = usePdfStore((s) => s.answerSentence);
  const answerSentences = usePdfStore((s) => s.answerSentences) || [];
  ...
  useEffect(() => {
    ...
    const highlightText = () => {
      ...
      const candidateQuotes = (quotes && quotes.length > 0) ? quotes : (quote ? [quote] : []);
      const candidateAnswers = (answerSentences && answerSentences.length > 0) ? answerSentences : (answerSentence ? [answerSentence] : []);

      if (candidateQuotes.length === 0 && candidateAnswers.length === 0 && !fallbackText) return;
      ...
      spans.forEach((span) => {
        const text = span.textContent.normalize("NFKD");
      ...
      const spanInfo = spans.map((span) => ({
        span,
        text: span.textContent.normalize("NFKD").replace(/\s+/g, ' ').trim().toLowerCase(),
      }));
      let combinedText = '';
      const spanRanges = [];
      ...
      const matchAndHighlightVerbatim = (phrase) => {
        if (!phrase) return false;
        const cleanPhrase = normalizeText(phrase);
        if (!cleanPhrase) return false;
        const matchIdx = normalizedText.indexOf(cleanPhrase);
        if (matchIdx !== -1) {
          const matchedSpans = new Set();
          for (let k = matchIdx; k < matchIdx + cleanPhrase.length; k++) {
            if (charIndexToSpan[k]) {
      ...
      for (const q of candidateQuotes) {
        if (matchAndHighlightVerbatim(q)) return;
      }

      // 2. Legacy answerSentence matching (exact character-level match, then fuzzy word matches)
      for (const ans of candidateAnswers) {
        if (!ans) continue;
        if (matchAndHighlightVerbatim(ans)) return;

        const cleanAnswer = ans.normalize("NFKD").replace(/[*_`]/g, '').replace(/\s+/g, ' ').trim().toLowerCase();
      ...
      // 3. Fallback matching for fallbackText (exact character-level match, then fuzzy word matches)
      if (fallbackText) {
        if (matchAndHighlightVerbatim(fallbackText)) return;
  ...
  }, [quote, quotes, answerSentence, answerSentences, fallbackText, currentPage, loading, storePage]);
